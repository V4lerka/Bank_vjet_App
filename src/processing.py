...
и возвращает новый список, отсортированный по дате (date)"""
    ...
    if len(new_list_operations) == 0:
        ...
        sorted_operations = (sorted(new_list_operations, key=lambda operations: operations.get("date"), reverse=True))
    elif not descending_order:
        sorted_operations = (sorted(new_list_operations, key=lambda operations: operations.get("date")))
    return sorted_operations
